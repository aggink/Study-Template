...
public sealed class SemaphoreService : IPrimeCounter
{
    ...
    public PrimeCountResultDto CountPrimes(int start, int end, int threadCount)
    {
        ...
        for (int i = 0; i < threadCount; i++)
        {
            ...
            {
                ProcessRange(threadNumber, tStart, tEnd, semaphore, ref primeCount, foundPrimes, showEveryCheck);
            });
    ...
    private void ProcessRange(int threadNumber, int start, int end, Semaphore semaphore, ref int primeCount, List<int> foundPrimes, bool showEveryCheck)
    {
