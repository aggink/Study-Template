...
public sealed class MutexService : IPrimeCounter, IDisposable
{
    private readonly Mutex _mutex = new();
    private readonly object _consoleLocker = new();

    public PrimeCounterResult CountPrimes(
        int startNumber,
        int endNumber,
        int threadCount,
        bool verbose = false)
    {
        ...
        for (int i = 0; i < threadCount; i++)
        {
            ...
            Thread thread = new(() =>
            {
                ...
                {
                    bool isPrime = MonitorService.IsPrime(number);
