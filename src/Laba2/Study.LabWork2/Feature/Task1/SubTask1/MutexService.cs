...
public sealed class MutexService : IPrimeCounter
{
    ...
    public PrimeCountResultDto CountPrimes(int start, int end, int threadCount)
    {
        ...
        for (int i = 0; i < threadCount; i++)
        {
            ...
            {
                ProcessRange(threadNumber, tStart, tEnd, mutex, ref primeCount, foundPrimes, showEveryCheck);
            });
    ...
    private void ProcessRange(int threadNumber, int start, int end, Mutex mutex, ref int primeCount, List<int> foundPrimes, bool showEveryCheck)
    {
